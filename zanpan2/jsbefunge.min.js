...
* jsBefunge
* 
* Copyright (C) 2013 STERS All rights reserved.
* 
* This program licensed GPL v3.
* All message is here http://www.gnu.org/licenses/gpl.html
*
*/
var jsBefunge={};(function($jb){$jb.EventManage=function(){this.events={};};$jb.EventManage.prototype.on=function(target,func){if(!this.events[target])
this.events[target]=[];this.events[target].push(func);return this.events[target].length-1;};$jb.EventManage.prototype.off=function(target,eventID){if(!this.events[target])
return;this.events[target][eventID]=null;};$jb.EventManage.prototype.fire=function(target,_this,args){if(!this.events[target])
return;for(var _i=0,_m=this.events[target].length;_i<_m;_i++){var event=this.events[target][_i];event.call(_this,_this,args);}};$jb.EventManage.prototype.defineFire=function(clsname,functions){for(var _key in functions){(function(_item){eval("$jb."+clsname+".prototype."+_key+"= function() {"+"var _v = _item.apply(this, arguments);"+"this._event.fire('"+clsname+"."+_key+"', this, arguments);"+"$jb.Event.fire('"+clsname+"."+_key+"', this, arguments);"+"return _v;"+"};");})(functions[_key]);}};$jb.Event=new $jb.EventManage();$jb.Buffer=function(){this.pos=0;this.buffer="";this._event=new jsBefunge.EventManage();};$jb.Buffer.prototype.clear=function(){this.buffer="";this.pos=0;};$jb.Buffer.prototype.set=function(str){this.buffer=str;this.pos=0;};$jb.Buffer.prototype.add=function(str){this.buffer+=str;};$jb.Buffer.prototype.get=function(nonMove){if(this.buffer.length<=this.pos)
return"";var c=this.buffer.charAt(this.pos);if(!nonMove)
this.pos++;return c;};$jb.InputBuffer=function(){$jb.Buffer.apply(this);};$jb.InputBuffer.prototype=new $jb.Buffer();$jb.InputBuffer.prototype.__super=$jb.Buffer.prototype;var fnames=["clear","set"]
...
this.c=this.source[0].charAt(0);this.eventFire("Code.init",this);};$jb.Code.prototype.next=function(){if(!this.running)
return;this.pos[0]+=this.direction==4?-1:(this.direction==6?1:0);this.pos[1]+=this.direction==8?-1:(this.direction==2?1:0);if(this.pos[0]>this.lenmax-1)
this.pos[0]=0;if(this.pos[0]<0)
this.pos[0]=this.lenmax-1;if(this.pos[1]>this.source.length-1)
this.pos[1]=0;if(this.pos[1]<0)
this.pos[1]=this.source.length-1;this.c=this.source[this.pos[1]].charAt(this.pos[0]);};$jb.Code.prototype.run=function(src,initOnly){var _this=this;if(_this.running==false){_this.stop(true);_this.init(src);_this.running=true;}
...
$jb.Code.prototype._step=function(){if(!this.running)
return false;if(this.strings){if(this.c!='"'){this.Stack.push(this.c.charCodeAt(0));this.next();}else{this.next();this.strings=false;}
return true;}
